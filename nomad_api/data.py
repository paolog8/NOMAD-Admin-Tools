...
def get_all_samples_with_authors(
    ...
    for owner in ["admin", "visible"]:
        ...
        try:
            if show_progress:
                print(f"Attempting to retrieve samples with {owner} access...")
            ...
            # If we got here without errors, we successfully retrieved data
            ...
            if owner == "admin":
                ...
                # Reset pagination for next attempt
                page = 1
                total_pages = float('inf')
                continue
            else:
                # Re-raise the error if visible also fails
                raise
...
def query_sample_entries(client: NomadClient, query: Dict[str, Any] = None, 
    ...
    for owner in ["admin", "visible"]:
        ...
        try:
            ...
            # If admin fails, continue to try visible
            if owner == "admin":
                continue
            else:
                raise  # Re-raise the error if visible also fails
