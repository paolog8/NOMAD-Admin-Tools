...
# Import the NomadClient for type annotations
...
def get_user_details(client: NomadClient, user_id: str) -> Dict[str, Any]:
    """
...
def get_hysprint_data(client: NomadClient, max_entries: Optional[int] = 500) -> Optional[pd.DataFrame]:
    """
    ...
    """
    try:
        # Try admin first, then fall back to visible
        for owner in ["admin", "visible"]:
            query_payload = {
                "owner": owner,
                "query": {
                    "and": [
            ...
            try:
                ...
                if owner == "admin":
                    ...
                    print("Falling back to visible access...")
                    continue
                else:
                    raise  # Re-raise if visible also fails
                
        return None  # Return None if both admin and visible fail
